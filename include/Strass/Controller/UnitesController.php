...
class UnitesController extends Strass_Controller_Action
{
    function indexAction()
    {
        ...
        if ($u->isFermee())
            $this->actions->append(
                "Rouvrir", array('action' => 'inscrire'), array(null, $u));

        $soustypename = $u->getSousTypeName();
        if (!$u->isTerminale() && $soustypename)
            $this->actions->append(
                "Fonder une ".$soustypename,
                array('action' => 'fonder'), array(null, $u));
        ...
        $this->actions->append(
            "Éditer l'unité", array('action' => 'editer'), array(null, $u));

        $this->actions->append(
            "Paramétrer la page",
            array('action' => 'parametres'), array(null, $u));

        if (!$u->isFermee())
            $this->actions->append(
                "Fermer l'unité",
                array('action' => 'fermer'), array(null, $u));
    }
    ...
    function effectifsAction()
    {
        ...
        if (!$u->findParentTypesUnite()->virtuelle)
            $this->actions->append(
                "Inscrire", array('action' => 'inscrire'), array(null, $u));
    ...
    function inscrireAction()
    {
        ...
        $this->metas(array(
            'DC.Title' => "Inscrire pour l'année $a-".($a+1),
