...
SELECT DISTINCT
        ...
        || ' ' || group_concat(coalesce(a.titre, ''), ' ')
        || ' ' || group_concat(coalesce(u.nom, ''), ' ')
        || ' ' || group_concat(coalesce(u.extra, ''), ' ')
        || ' ' || group_concat(coalesce(t.nom, ''), ' ')
        || ' ' || group_concat(coalesce(r.slug, ''), ' ')
        || ' ' || group_concat(coalesce(r.accr, ''), ' ')
        AS content
FROM individu
...
LEFT OUTER JOIN unite_role AS r ON r.id = a.`role`
